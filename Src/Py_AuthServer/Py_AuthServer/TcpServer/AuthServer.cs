...
namespace Py_AuthServer
{
    public class AuthServer
    {
        ...
        public virtual void DisconnectClient(AuthClient Server)
        {
            ...
            int logincount = Clients.Model.Where(c => c.Data.Type == AuthClientTypeEnum.LoginServer).ToList().Count;
            int gamecount = Clients.Model.Where(c => c.Data.Type == AuthClientTypeEnum.GameServer).ToList().Count;
            int messengercount = Clients.Model.Where(c => c.Data.Type == AuthClientTypeEnum.MessengerServer).ToList().Count;
            Console.Title = string.Format("Pangya Fresh Up! AuthServer - LOGIN: {0}, GAMES: {1}, MESSENGER: {2}",logincount, gamecount, messengercount);
            UpdateServer();
        }
